...
class BluetoothSerialTemplate extends React.Component {
  constructor(props) {
    ...
    BluetoothActions.setFunctions({
      ...
      activateDevice: this.activateDevice.bind(this),
      disconnectDevices: this.disconnectDevices.bind(this),
    });
  ...
  writeTemperatureControlDevices(readData) {
    ...
    if (
      readData.humid <= LOW_HUMIDITY &&
      this.props.autoHumidifier === ETC.status.running
    ) {
      this.write("b"); //켜짐
    ...
    if (
      readData.temp >= HIGH_TEMPERATURE &&
      this.props.autoCoolFan === ETC.status.running
    ) {
      this.write("c"); //켜짐
    ...
    if (
      ...
      for (let i = 0; i < devices.length; i++) {
        if (isNotNull(devices[i])) {
          const res = checkDuplicate.find(
            deviceId => deviceId === devices[i].device
          );
          if (!isNotNull(res)) {
